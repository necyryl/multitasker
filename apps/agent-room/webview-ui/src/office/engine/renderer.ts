...
function getSceneBounds(
  ...
  for (const f of furniture) {
    const width = f.sprite[0]?.length ?? TILE_SIZE;
    const height = f.sprite.length || TILE_SIZE;
    bounds = expandBounds(bounds, f.x, f.y, f.x + width, f.y + height);
  }

  for (const ch of characters) {
    bounds = expandBounds(bounds, ch.x - 10, ch.y - 28, ch.x + 10, ch.y + 4);
  }

  if (!bounds) {
    bounds = { minX: 0, minY: 0, maxX: cols * TILE_SIZE, maxY: rows * TILE_SIZE };
  }

  const padX = TILE_SIZE;
  const padTop = TILE_SIZE * 2;
  const padBottom = TILE_SIZE;
  return {
    minX: Math.max(0, bounds.minX - padX),
    minY: Math.max(0, bounds.minY - padTop),
    maxX: Math.min(cols * TILE_SIZE, bounds.maxX + padX),
    maxY: Math.min(rows * TILE_SIZE, bounds.maxY + padBottom),
  };
}
...
export function getSceneOffset(
  ...
  const bounds = getSceneBounds(tileMap, furniture, characters, layoutCols, layoutRows);
  const focusW = (bounds.maxX - bounds.minX) * zoom;
  const focusH = (bounds.maxY - bounds.minY) * zoom;
  return {
    offsetX:
      Math.floor((canvasWidth - focusW) / 2) -
      Math.round(bounds.minX * zoom) +
      Math.round(panX),
    offsetY:
      Math.floor((canvasHeight - focusH) / 2) -
      Math.round(bounds.minY * zoom) +
      Math.round(panY),
  };
}
...
export function renderFrame(
  ...
  const { offsetX, offsetY } = getSceneOffset(
    canvasWidth,
    canvasHeight,
    tileMap,
    allFurniture,
