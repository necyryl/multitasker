    ...
    <>
      ...
      {showUpdateNotice && (
        <div
          ...
          className="absolute bottom-46 right-10 z-20 pixel-hud px-10 pt-8 pb-9 cursor-pointer flex flex-col gap-8 max-w-2xs"
          style={{
            opacity: fading ? 0 : 1,
      ...
      {!showUpdateNotice && labelHovered && (
        <div
          onClick={handleOpenChangelog}
          className="absolute bottom-46 right-10 z-20 pixel-hud py-5 px-10 cursor-pointer text-sm whitespace-nowrap"
        >
          See what's new!
        </div>
      )}
      ...
      <div
        ...
        onClick={handleOpenChangelog}
        className="absolute bottom-10 right-10 z-20 pixel-hud py-3 px-8 text-sm cursor-pointer select-none transition-opacity duration-200"
        style={{ opacity: labelHovered ? 1 : 0.75 }}
      >
        v{currentMajorMinor}
      </div>
    </>
