...
let mockPayloadPromise: Promise<void> | null = null;
...
function dispatchMockShowroom(dispatch: (data: unknown) => void): void {
  dispatch({
  ...
  dispatch({
    ...
    folders: [
      { name: 'pixel-agents', path: 'C:\\Users\\Kyrylo\\pixel-agents' },
      { name: 'renderer', path: 'C:\\Users\\Kyrylo\\pixel-agents\\webview-ui' },
    ],
  });
  dispatch({
    type: 'existingAgents',
    agents: [1, 2, 3, 4],
    agentMeta: {
      1: { palette: 0, hueShift: 80 },
      2: { palette: 2, hueShift: 170 },
      3: { palette: 4, hueShift: 260 },
      4: { palette: 5, hueShift: 40 },
    },
    folderNames: {
      1: 'pixel-agents',
      2: 'webview-ui',
...
function dispatchMockAgentActivity(dispatch: (data: unknown) => void): void {
  dispatch({
    type: 'agentToolStart',
    id: 1,
    toolId: 'mock-tool-renderer',
    toolName: 'Edit',
    status: 'Refining room',
  });
  ...
  dispatch({
    type: 'agentToolStart',
    id: 3,
    toolId: 'mock-tool-read',
    toolName: 'Read',
    status: 'Waiting',
  });
  dispatch({ type: 'agentStatus', id: 3, status: 'waiting' });

  dispatch({
    type: 'agentToolStart',
    id: 4,
    toolId: 'mock-tool-test',
    toolName: 'Test',
    status: 'Build passed',
  });
  dispatch({ type: 'agentToolDone', id: 4, toolId: 'mock-tool-test' });
...
export async function initBrowserMock(): Promise<void> {
  ...
  if (mockPayloadPromise) return mockPayloadPromise;

  mockPayloadPromise = loadBrowserMock().catch((err) => {
    mockPayloadPromise = null;
    throw err;
  });
  await mockPayloadPromise;
}

async function loadBrowserMock(): Promise<void> {
...
export function dispatchAssetMessages(dispatch: (data: unknown) => void): void {
  if (!mockPayload) return;
...
export function dispatchMockMessages(): void {
  ...
  dispatchMockShowroom(dispatch);
  dispatchAssetMessages(dispatch);
  dispatchMockAgentActivity(dispatch);
